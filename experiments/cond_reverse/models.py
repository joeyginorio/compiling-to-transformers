...
# Dictionary-based conditional reverse: if the first element of the list is a
# "vowel" (branch 0 of the element sum), reverse the list; otherwise return it
# unchanged. Element width is set to d_head so each sequence element's type
# matches the attention head dimension.
case_vars = [f'_v{i}' for i in range(10)]
branches: list[Tm] = [
    TmSeq(TmVar(case_vars[i]), rev('xs2') if i == 0 else TmVar('xs2'))
    for i in range(10)
]
conditional_reverse_attn = TmCase(
    TmProj(0, TmVar('xs1')),
    case_vars,
    branches,
)
check(conditional_reverse_attn, {
    'xs1': TyProd([elem_ty] * 15),
    'xs2': TyProd([elem_ty] * 15),
})

# NOTE: Set the programmed module here.
program_module = cj.compile(conditional_reverse_attn)
...
class ModelT(nn.Module):
    def __init__(self, vocab_size: int = 27, d_model: int = 20, n_heads: int = 2, seq_len: int = 30):
        ...
        d_head = d_model // n_heads
        self.prog = cj.to_multilinear(program_module, rand=True, sample_env={'xs1': torch.zeros(SEQ_LEN * d_head),
                                                                             'xs2': torch.zeros(SEQ_LEN * d_head)})
        self.prog_proj_in = nn.Linear(d_model, d_head, bias=False)
        self.merge = nn.Linear(d_model + d_head, d_model, bias=False)
