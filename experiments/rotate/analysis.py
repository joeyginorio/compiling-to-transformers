...
from experiments.rotate.models import ModelF, ModelT, ModelI, ModelU
from experiments.rotate.dataset import datasets, decode, SEQ_LEN
from experiments.rotate.learning import evaluate, _seeds, DEVICE
...
_data_dir = Path("experiments/rotate/data/learning")
...
def add_test_loss(lr: float, batch_size: int, max_step: int = 5000) -> None:
    ...
    test_steps = range(0, max_step + 1, 100)
...
def plot_behavioral_dynamics(lr: float | None = None, batch_size: int | None = None, max_step: int = 800, test: bool = False):
    ...
    for model_name in models:
        ...
        run_df["output2"] = run_df["outputs"].apply(lambda s: ast.literal_eval(s)[0])  # type: ignore
...
def plot_pca(model: nn.Module, step: int, lr: float, batch_size: int, seed: int,
    ...
    sns.scatterplot(data=plot_df, x="PC1", y="PC2", hue=hue_col, hue_order=char_order,  # type: ignore[arg-type]
                    palette="turbo", alpha=0.5, s=20, marker="o",
                    edgecolor="none", ax=ax1, legend=False)
    for char, group in plot_df.groupby(hue_col):
    ...
    sns.scatterplot(data=plot_df, x="PC1", y="PC2", hue="Position", hue_order=pos_order,  # type: ignore[arg-type]
                    palette="turbo", alpha=0.5, s=20, marker="o",
...
def plot_pca_all(models_list: list[nn.Module], step: int, lr: float, batch_size: int, seed: int,
    ...
    for col, model in enumerate(models_list):
        ...
        sns.scatterplot(data=plot_df, x="PC1", y="PC2", hue="Position", hue_order=pos_order,  # type: ignore[arg-type]
                        palette="turbo", alpha=0.5, s=20, marker="o",
                        edgecolor="none", ax=ax2, rasterized=True)
        ax2.set_anchor("N")
        if col == 0:
...
def plot_pca_res(models_list: list[nn.Module], step: int, lr: float, batch_size: int, seed: int,
    ...
    for col, model in enumerate(models_list):
        ...
        for row, (rep, hue_col) in enumerate(row_configs):
            ...
            colors = [color_map[c] for c in plot_df[hue_col]]
            sc = ax.scatter(plot_df["PC1"], plot_df["PC2"], plot_df["PC3"],
                            c=colors, alpha=0.5, s=80, marker="o",
            ...
            for char in plot_df[hue_col].unique():
                ...
                if len(char_df) == 0:
                    continue
                cx = float(char_df["PC1"].mean())
                cy = float(char_df["PC2"].mean())
                cz = float(char_df["PC3"].mean())
...
def prop_correct(lr: float, batch_size: int) -> pd.DataFrame:
    ...
    for model_name in models:
        ...
        df["output2"] = df["outputs"].apply(lambda s: ast.literal_eval(s)[0])  # type: ignore
        for i, ch in enumerate("abcdefghijklmno"):
