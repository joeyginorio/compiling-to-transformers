...
class ModelU(nn.Module):
    def __init__(self, model_d: nn.Module):
        super().__init__()
        self.model = cj.to_multilinear(model_d)
    
    def forward(self, env: cj.Env) -> torch.Tensor:
        return self.model(env)

class ModelT(nn.Module):
    def __init__(self, model_d: nn.Module):
        super().__init__()
        self.model = cj.to_multilinear(model_d, rand=True)
